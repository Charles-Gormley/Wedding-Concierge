...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!authHeader || authHeader !== `Bearer ${API_KEY}`) {
      console.log("Unauthorized access attempt to Storage Test API")
    ...
    if (!tableName) {
      return NextResponse.json({ error: "Storage configuration error" }, { status: 400 })
    }
    ...
    console.error("Error in Storage Test API:", error)
    return NextResponse.json(
      {
        success: false,
        error: "Storage connection test failed",
